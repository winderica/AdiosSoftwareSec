...
class HOFinder {
    ...
    void traverse(const json &source) {
        ...
        if (kind == "Program") {
            ...
            string lowercase;
            for (auto i:name) {
                lowercase.push_back(tolower(i));
            }
            if (lowercase.find("heap") != string::npos || lowercase.find("alloc") != string::npos) {
                hasHeapFunctions = true;
            }
            if (find(vulnerable.begin(), vulnerable.end(), name) != vulnerable.end() && hasHeapFunctions) {
                json data = json(
                    {
                        {"function",   name},
...
public:
    explicit HOFinder(json ast) : ast(move(ast)) {}
...
json findHO(string code, const vector<string> &types) {
    auto tokens = Lexer(move(code), types).tokenize();
    auto ast = Parser(tokens).parse();
    return HOFinder(ast).findVulnerable();
}

Napi::Value HeapOverflow(const Napi::CallbackInfo &info) {
    ...
    try {
        ...
        return Napi::String::New(env, findHO(code, types).dump());
    } catch (exception &e) {
