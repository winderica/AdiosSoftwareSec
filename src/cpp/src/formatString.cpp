...
class FSFinder {
    ...
    json undefinedStrings = json::array();
        ...
        if (kind == "Program") {
            ...
            if (kind.find("Array") != string::npos) {
                ...
                if (kind.find("Definition") == string::npos) {
                    if (source["type"]["name"] == "char") {
                        undefinedStrings.push_back(source["identifier"]["name"]);
                    }
                }
            ...
            if (name == "printf") {
                ...
                unsigned long l0, l1;
                l1 = source["arguments"].size() - 1;
                if (source["arguments"][0]["kind"] == "Identifier") {
                    string argument = source["arguments"][0]["name"];
                    if (find(undefinedStrings.begin(), undefinedStrings.end(), argument) != undefinedStrings.end()) {
                        data["description"] = "printf with unknown source string " + argument + " is unsafe";
                        result.push_back(data);
                    }
                } else if (source["arguments"][0]["kind"] == "StringLiteral") {
                    string value = source["arguments"][0]["value"];
                    regex re(R"(%(?:\+|-| |#|0)?[\d*]?(?:.\d|.\*)?(?:hh|h|l|ll|L|z|j|t|I|I32|I64|q)?[%diufFeEgGxXoscpaAn])");
                    l0 = 0;
                    std::sregex_iterator iter(value.begin(), value.end(), re);
                    ...
                    if (l0 > l1) {
                        data["description"] =
                            name + " is unsafe: format string length " + to_string(l0) + " is longer than parameters length " + to_string(l1);
                        result.push_back(data);
                    } else if (value.find("%n") != string::npos) {
                        data["description"] = "%n is unsafe";
                        result.push_back(data);
                    }
                }
            }
...
public:
    explicit FSFinder(json ast) : ast(move(ast)) {}
...
json findFS(string code, const vector<string>& types) {
    auto tokens = Lexer(move(code), types).tokenize();
    auto ast = Parser(tokens).parse();
    return FSFinder(ast).findVulnerable();
}

Napi::Value FormatString(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        return Napi::String::New(env, findFS(code, types).dump());
    } catch (exception &e) {
